...
## Συνάρτηση προεπεξεργασίας της εικόνας εισόδου
...
model_filename = "testing.h5"
loaded_model = load_model(model_filename)
print("Model loaded successfully!")

## Δημιουργία παραθύρου Tkinter (GUI)
root = tk.Tk()
root.withdraw()

## Χρήση file dialog για την επιλογή εικόνας
input_image_path = filedialog.askopenfilename(title="Select an image", filetypes=[("Image files", "*.jpg *.png")])

## Έλεγχος αν έχει επιλεγεί μια εικόνα
if input_image_path:
    print("Selected image:", input_image_path)

    # Προ-επεξεργασία της εικόνας εισόδου και δημιουργία πρόβλεψης
    preprocessed_img = preprocess_image(input_image_path)
    preprocessed_img = np.expand_dims(preprocessed_img, axis=0)  # Add batch dimension
    predicted_class = np.argmax(loaded_model.predict(preprocessed_img))  # Get the predicted class index
    print(predicted_class)

    ## Ορισμός μηνυμάτων και αρχείων ήχου για κάθε κλάση
    class_messages = {
    ...
    ## Εκτύπωση του προβλεπόμενου μηνύματος
